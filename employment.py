...
def compute_department_stats(employees, min_salary, min_years):
    ...
    filtered = [e for e in employees if e.salary >= min_salary and e.years_of_service >= min_years]
    ...
    for dept, sals in depts.items():
        if sals:
            ...
            median = sals[len(sals) // 2] if len(sals) % 2 else (sals[len(sals) // 2 - 1] + sals[len(sals) // 2]) / 2
            stats[dept] = {'average': avg, 'median': median, 'count': len(sals)}
